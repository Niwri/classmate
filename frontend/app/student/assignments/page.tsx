...
export default function Page() {
  ...
  const handleSubmit = () => {
    if (file) {
      ...
      reader.onload = () => {
        ...
        router.push(
          `/student/assignments/evaluate?assignmentid=${assignmentList[assignmentId]["assignment-id"]}`,
        );
      };
    } else {
  ...
  const DropdownWindow: React.FC = () => {
    return (
      ...
      >
        <section
          className="w-fit h-fit bg-white p-12 rounded-3xl flex flex-col items-center gap-y-8"
          onClick={(event) => event.stopPropagation()}
        >
          <h1 className="font-semibold text-2xl">
            {assignmentList[assignmentId].name}
          </h1>
          <div
                  ...
                  <>
                    ...
                    >
                      <td className="font-semibold px-4 py-1 w-32 rounded-l-full group-hover:bg-sky-300 flex flex-col items-center justify-center">
                        {data.status == 1 ? (
                          <div className="bg-green-300 rounded-full text-center w-full py-1">
                            Complete
                          </div>
                        ) : (
                          <div className="bg-red-300 rounded-full text-center w-full py-1">
                            Incomplete
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-2 group-hover:bg-sky-300 font-semibold">
                        {data.name}
                      </td>
                      <td className="px-4 py-2 w-48 rounded-r-full group-hover:bg-sky-300">
                        {data.date}
                      </td>
                    </tr>
                    <tr>
